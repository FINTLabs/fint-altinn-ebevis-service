...
public class CertificateConverter {
    ...
    public byte[] convertTaxCertificate(Evidence evidence, AltinnApplication application) {
        try {
            ...
            document.add(new Paragraph()
                            ...
                            .add(new Text(" | "))
                            .add(new Link("Novari IKS", PdfAction.createURI("https://novari.no")))
    ...
    private Document createDocument(File pdfFile) throws IOException {
        ...
        try {
            ...
            String resourcePath = fontFile.startsWith("classpath:") ? fontFile.substring("classpath:".length()) : fontFile;
            ...
            try (InputStream fontStream = getClass().getClassLoader().getResourceAsStream(resourcePath)) {
                if (fontStream != null) {
                    ...
                    throw new IOException("Font resource not found: " + fontFile);
                }
            }
        } catch (Exception e) {
            log.error("Failed to load custom font, PDF/A compliance may fail. Error: {}", e.getMessage());
            // This will likely cause PDF/A compliance issues, but it's the only fallback
